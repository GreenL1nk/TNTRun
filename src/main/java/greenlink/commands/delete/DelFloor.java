...
public class DelFloor extends AbstractCommand {
    @Override
    protected void onPlayerCommand(@NotNull Player player, @NotNull String[] args) {
        ...
        if (arenaByName.removeFloorByNumber(Integer.parseInt(args[1]))) {
            player.sendMessage("Этаж " + args[1] + " успешно удалён");
        }
        else {
            player.sendMessage("Этажа " + args[1] + " нету в базе");
        }
    }
    ...
    protected List<String> onPlayerTab(@NotNull Player player, @NotNull String[] args) {
        ...
        if (args.length == 2) {
            Arena arenaByName = ArenaManager.getInstance().getArenaByName(args[0]);
            if (arenaByName == null) return List.of();
            return arenaByName.getFloors().stream().map(floor -> String.valueOf(floor.getNumber())).toList();
        }
        return List.of();
    }
