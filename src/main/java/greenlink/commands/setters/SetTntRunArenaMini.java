...
public class SetTntRunArenaMini extends AbstractCommand {
    @Override
    protected void onPlayerCommand(@NotNull Player player, @NotNull String[] args) {
        if (!player.isOp()) return;
        if (args.length == 0) return;

        if (ArenaManager.getInstance().tryAddArena(player.getWorld(), args[0], true)) {
            player.sendMessage("Этот мир установлен как малая арена с именем " + args[0]);
        }
        else {
